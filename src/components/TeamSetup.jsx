...
const SUGGESTIONS = ['Power Rangers', 'Mujeres Maravilla', 'Los Tigres', 'Las Panteras', 'Dragones', 'Fénix', 'Avengers', 'Titanes']
  ...
  return (
    ...
    >
      <header className="py-6 text-center">
        ...
        <p className="mt-2 text-2xl font-extrabold text-white/70 md:text-3xl">Equipo vs Equipo · Pantalla dividida</p>
      </header>
        ...
        {[0, 1].map((i) => (
          <div key={i} className={`flex flex-col items-center justify-center gap-8 p-8 ${i === 0 ? 'bg-teamA-dark/40 border-r-4 border-white/20' : 'bg-teamB-dark/40'}`}>
            ...
            <input
              ...
              maxLength={24}
              aria-label={`Nombre equipo ${i === 0 ? 'A' : 'B'}`}
      ...
      <footer className="flex items-center justify-center gap-6 py-8">
        ...
        <span className="text-xl text-white/50">Al empezar se activa el audio (Tone.js)</span>
      </footer>
    </form>
  )
}
